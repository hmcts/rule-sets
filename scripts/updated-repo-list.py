    ...
    if url.endswith('.yml') or url.endswith('.yaml'):
        ...
        for key in data:
            if isinstance(data[key], list):
                for item in data[key]:
                    if 'repo' in item:
                        repo_name = item['repo'].split('/')[-1].replace('.git', '')
        ...
        if 'module_calls' in data:
            for item in data['module_calls']:
                if 'source' in item:
                    # Extract the repository name from the 'source' value and remove the .git and ref=master
                    match = re.search(r'git@github.com:(.*?)(\.git|$)', item['source'])
                    if match:
                        repo_name = match.group(1).split('/')[-1].replace('.git', '')
                        all_repos.append(repo_name)
